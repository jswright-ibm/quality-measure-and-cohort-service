...
public class TranslationResource {
	...
	public Response cqlPackageToElmPackage(
		...
		try {
					...
					for( String format : targetFormats ) {
						for( String subformat : format.split(",") ) {
							...
							if (targetFormat.equals(MediaType.valueOf(ELM_XML_TYPE))) {
								...
								return Response.serverError().type(MediaType.TEXT_PLAIN_TYPE).entity("Unsupported media type").build();
							}
						}
					}
				}
			...
			return Response.serverError().type(MediaType.TEXT_PLAIN_TYPE).entity("Unable to read request").build();
		} finally {
			libraryManager.getLibrarySourceLoader().clearProviders();
		}
	}
	...
	private CqlTranslator getTranslator(File cql, MultivaluedMap<String, String> params) {
		try {
			...
			for (String key: params.keySet()) {
				if (PARAMS_TO_OPTIONS_MAP.containsKey(key) && Boolean.parseBoolean(params.getFirst(key))) {
					...
					try {
						...
						throw new IllegalArgumentException("Cannot load UCUM service to validate units", e);
					}
			...
			throw new IllegalArgumentException("Unable to read request", e);
		}
	}
